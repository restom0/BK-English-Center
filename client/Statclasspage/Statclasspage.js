...
function loadData() {
  ...
  apiRequest({
    type: "get",
    url: "/courses",
    success: function (res) { courseData = res.data; },
  });
  ...
  apiRequest({
    ...
    error: function (jqXHR) {
      Toast.fire({ icon: "error", title: jqXHR.responseJSON?.msg || i18n.t('validate.load_error') });
    },
  });
}

function buildCourseOptions(selectedName) {
  return courseData.map(function (el) {
...
function addData() {
  $(".submitAddBtn").click(function (e) {
    ...
    if (Object.values(payload).some(function (v) { return !v; })) {
...
function loadEditData() {
  $(".editBtn").click(function (e) {
    ...
    $(".closeBtn").click(function () { closeModal("#editModal"); });
    editData(id);
  });
}
  ...
  $(".deleteBtn").click(function (e) {
    ...
    Swal.fire({
      ...
      apiRequest({
        ...
        success: function () {
          Toast.fire({ icon: "success", title: i18n.t('toast.delete_ok') }).then(function () { loadData(); });
        },
        error: function (jqXHR) {
          Toast.fire({ icon: "error", title: jqXHR.responseJSON?.msg });
        },
