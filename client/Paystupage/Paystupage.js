    ...
    if ($("#morePay").html() === i18n.t('action.view_more')) {
      ...
      tempPay.forEach((el, index) => {
        ...
        if (Number(el["paidStatus"]) === 0) {
          payStr += i18n.t('status.not_closed');
        } else {
          payStr += i18n.t('status.closed');
        }
        payStr +=
          `
            </td>
...
function loadData() {
  ...
  $.ajax({
    type: "get",
    url: API_URL + "/studentjoinclasses/null-prize",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("apitoken"),
    },
    dataType: "JSON",
    ...
    str += `
          ...
          <div>
              <label for="classPrize"
                  class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">` + i18n.t('label.class') + `</label>
...
function editPayModal() {
  $(".editPayBtn").click(function (e) {
    ...
    str +=
      ...
      <div>
      <label for="paidStatus" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">` + i18n.t('label.status') + `</label>
    ...
    str += `>` + i18n.t('status.not_closed') + `</option>
        <option value="1"`;
    if (tempPay[id]["paidStatus"] == 1) {
      str += "selected";
    }
    str += `>` + i18n.t('status.closed') + `</option>
      </select>
      </div>
  </div> 
  </form>
  ...
  $(".submitEditPayBtn").click(function (e) {
    ...
    if (!status || status === "") {
      ...
      Swal.fire({
        title: i18n.t('confirm.title'),
        text:
          i18n.t('confirm.editing_class', {name: tempPay[i]["name"], class: tempPay[i]["className"]}),
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
...
function deletePay() {
  $(".deletePayBtn").click(function (e) {
    ...
    Swal.fire({
      title: i18n.t('confirm.title'),
      text:
        i18n.t('confirm.deleting_class', {name: tableData[id]["name"], class: tableData[id]["payClass"]}),
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
  ...
  $(".submitEditPrizeBtn").click(function (e) {
    ...
    if (!status || status === "") {
      ...
      Swal.fire({
        title: i18n.t('confirm.title'),
        text:
          i18n.t('confirm.editing_class', {name: tempPrize[i]["name"], class: tempPrize[i]["className"]}),
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
...
function deletePrize() {
  $(".deletePrizeBtn").click(function (e) {
    ...
    Swal.fire({
      title: i18n.t('confirm.title'),
      text:
        i18n.t('confirm.deleting_class', {name: tempPrize[id]["name"], class: tempPrize[id]["className"]}),
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
