...
function loadData() {
  ...
  apiRequest({
    ...
    success: function (res) {
      classData = res.data;
      var rows = (res?.data ?? []).map(function (el, index) {
        return (
...
function buildCourseOptions(selectedName) {
  return courseData
    .map(function (el) {
      var selected = el.name === selectedName ? 'selected' : '';
  ...
  $('.addModal').click(function (e) {
    ...
    var str =
      ...
      <div class="gap-4 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <form id="addForm">
          <div class="grid gap-6 mb-6 md:grid-cols-4">
            <div>
              <label for="className" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">` +
      ...
      `</option>
            ...
            <div>
              <label for="maxStudent" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">` +
      i18n.t('label.max_students') +
      `</label>
      ...
      `</label>
        ...
        <div class="mt-6 w-full flex justify-between">
          <button class="closeBtn px-5 py-2.5 text-sm font-medium text-white bg-red-500 rounded-lg hover:bg-red-600">` +
      i18n.t('action.cancel') +
      `</button>
          <button form="addForm" type="submit" class="submitAddBtn px-5 py-2.5 text-sm font-medium text-white bg-green-500 rounded-lg hover:bg-green-600">` +
      i18n.t('action.add') +
      `</button>
        </div>
      </div>`;
...
function loadEditData() {
  $('.editBtn').click(function (e) {
    ...
    var cls = classData[id];
    var startdate = toLocalDate(cls.startDate);
    var enddate = toLocalDate(cls.endDate);
    ...
    var str =
      ...
      i18n.t('action.cancel') +
      `</button>
          <button form="editForm" type="submit" class="submitEditBtn px-5 py-2.5 text-sm font-medium text-white bg-green-500 rounded-lg hover:bg-green-600">` +
      i18n.t('action.save') +
      `</button>
        </div>
      </div>`;

    $('#editModal').html(str).removeClass('invisible opacity-0').addClass('opacity-100');
    $('.closeBtn').click(function () {
