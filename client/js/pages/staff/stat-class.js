...
function loadData() {
  ...
  apiRequest({
    ...
    error: function (jqXHR) {
      Toast.fire({
        icon: 'error',
        title: (jqXHR.responseJSON?.msg ?? 'Lỗi kết nối máy chủ') || i18n.t('validate.load_error'),
      });
    },
  });
}
      ...
      <div class="gap-4 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <form id="addForm">
          <div class="grid gap-6 mb-6 md:grid-cols-4">
            <div>
              <label for="className" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">${i18n.t(
                'label.class_name'
              )}</label>
            ...
            <div>
              <label for="courseName" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">${i18n.t(
                'label.course'
              )}</label>
            ...
            <div>
              <label for="maxStudent" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">${i18n.t(
                'label.max_students'
              )}</label>
            ...
            <div>
              <label for="schedule" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">${i18n.t(
                'label.schedule'
              )}</label>
            ...
            <div>
              <label for="address" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">${i18n.t(
                'label.branch'
              )}</label>
            ...
            <div>
              <label for="startDate" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">${i18n.t(
                'label.start_date'
              )}</label>
            ...
            <div>
              <label for="endDate" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">${i18n.t(
                'label.end_date'
              )}</label>
        ...
        <div class="mt-6 w-full flex justify-between">
          <button class="closeBtn px-5 py-2.5 text-sm font-medium text-white bg-red-500 rounded-lg hover:bg-red-600">${i18n.t(
            'action.cancel'
          )}</button>
          <button form="addForm" type="submit" class="submitAddBtn px-5 py-2.5 text-sm font-medium text-white bg-green-500 rounded-lg hover:bg-green-600">${i18n.t(
            'action.add'
          )}</button>
        </div>
      </div>`;
    ...
    const str = `
      <div class="mt-5 gap-4 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <form id="editForm">
          <div class="grid gap-6 mb-6 md:grid-cols-4">
            <div>
              <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">${i18n.t(
                'label.class_name'
              )}</label>
        ...
        <div class="mt-6 w-full flex justify-between">
          ...
          <button form="editForm" type="submit" class="submitEditBtn px-5 py-2.5 text-sm font-medium text-white bg-green-500 rounded-lg hover:bg-green-600">${i18n.t(
            'action.save'
          )}</button>
        </div>
      </div>`;
