...
function editData() {
  $('.edit').click(function (e) {
    ...
    const idCourse = $(this).attr('data-value');
...
<button id="submitEditBtn"
    ...
    $('#submitEditBtn').click(function (e) {
      ...
      const dob = $('#dob').val();
      const sex = $("input[name='inline-radio-group']:checked").val();
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: 'patch',
            url: `${API_URL}/${bkProfileSettings.updateEndpoint}`,
            headers: {
...
<form>
    <div>
        <div class="gap-6 mb-6">
            ...
            <input type="text" id="full_name"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="${i18n.t('placeholder.person_name')}" required>
        </div>
    </div>
    <div class="grid gap-6 mb-6 md:grid-cols-2">
        <div>
...
function loadData() {
  $.ajax({
    type: 'get',
    url: `${API_URL}/${bkProfileSettings.listEndpoint}`,
    headers: {
      Authorization: `Bearer ${localStorage.getItem('apitoken')}`,
    },
    dataType: 'JSON',
...
window.BkProfilePage = {
  init: bkInitProfilePage,
};
