...
function bkInitAttendancePage(config) {
  ...
  $.ajax({
    type: 'get',
    url: `${API_URL}/${bkAttendanceSettings.peopleEndpoint}`,
    headers: {
...
function loadData() {
  $.ajax({
    type: 'get',
    url: `${API_URL}/${bkAttendanceSettings.joinClassEndpoint}/date`,
    headers: {
      Authorization: `Bearer ${localStorage.getItem('apitoken')}`,
    },
    dataType: 'JSON',
  ...
  $.ajax({
    ...
    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-center">
                    ...
                    `;
        if (el['status'] == 1) {
          str += i18n.t(bkAttendanceSettings.statusLabels.completed);
        }
        if (el['status'] == 0) {
          str += i18n.t(bkAttendanceSettings.statusLabels.active);
        } else if (el['status'] == -1) {
          str += i18n.t(bkAttendanceSettings.statusLabels.missing);
        }
        str += `
                    </td>`;
        if (bkAttendanceSettings.extraPersonColumnField) {
          str += `<td class="px-6 py-4">
                    ${el[bkAttendanceSettings.extraPersonColumnField]}
                    </td>`;
        }
        str += `
                    <td class="px-6 py-4">
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          error: function (jqXHR, textStatus, errorThrown) {
            Toast.fire({
              icon: 'error',
              title: jqXHR.responseJSON?.msg ?? 'LÃ¡Â»â€”i kÃ¡ÂºÂ¿t nÃ¡Â»â€˜i mÃƒÂ¡y chÃ¡Â»Â§',
            });
          },
        });
      }
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <label for="name"
                        ...
                        <option selected>${i18n.t(bkAttendanceSettings.selectPersonLabelKey)}</option>`;
...
function addDate() {
  $('.submitAddBtn').click(function (e) {
    ...
    if (!className || className === '') {
      ...
      $.ajax({
        type: 'post',
        url: `${API_URL}/${bkAttendanceSettings.joinClassEndpoint}/class`,
        headers: {
          Authorization: `Bearer ${localStorage.getItem('apitoken')}`,
        },
        data: {
          [bkAttendanceSettings.personIdField]: name,
          idClass: className,
        },
        dataType: 'JSON',
        success: function (res) {
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-3">
                ...
                <div>
                    <label for="status"
                        ...
                        }>${i18n.t(bkAttendanceSettings.statusLabels.missing)}</option>
                        <option value="0" ${
                          temp[id]['status'] === 0 ? 'selected' : ''
                        }>${i18n.t(bkAttendanceSettings.statusLabels.active)}</option>
                        <option value="1" ${
                          temp[id]['status'] === 1 ? 'selected' : ''
                        }>${i18n.t(bkAttendanceSettings.statusLabels.completed)}</option>
...
window.BkAttendancePage = {
  init: bkInitAttendancePage,
};
