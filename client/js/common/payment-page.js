(function (global) {
      ...
      $('#morePay').click(function (e) {
        ...
        if ($('#morePay').text() === i18n.t('action.view_more')) {
          ...
          tempPay.forEach((el, index) => {
            ...
            <span data-edit=${index} data-id=${el[settings.personIdField]} class="studentModal">
                        ${el['name']}
                    </span>
            </th>
            <td class="px-6 py-4">
            ...
            if (Number(el['paidStatus']) === 0) {
              payStr += i18n.t(settings.paidStatusLabels.unpaid);
            } else {
              payStr += i18n.t(settings.paidStatusLabels.paid);
            }
            payStr += `
            </td>
            <td class="px-6 py-4 flex justify-center">
                <button data-id=${el[settings.personIdField]} data-tooltip-target="notification"
      ...
      $('#morePrize').click(function (e) {
        if ($('#morePrize').text() === i18n.t('action.view_more')) {
          ...
          tempPrize.forEach((el, index) => {
            ...
            <td class="px-6 py-4 flex justify-center">
                <button data-id=${el[settings.personIdField]} data-tooltip-target="notification1"
  ...
  <div id="delete1" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
    ...
    function loadData() {
      ...
      $.ajax({
        type: 'get',
        url: `${API_URL}/${settings.joinClassEndpoint}/null-prize`,
        headers: {
          Authorization: `Bearer ${localStorage.getItem('apitoken')}`,
        },
        dataType: 'JSON',
      ...
      $.ajax({
        type: 'get',
        url: `${API_URL}/${settings.joinClassEndpoint}/salary`,
        headers: {
          Authorization: `Bearer ${localStorage.getItem('apitoken')}`,
        },
        dataType: 'JSON',
    ...
    <div class="tooltip-arrow" data-popper-arrow></div>
      ...
      $.ajax({
        type: 'get',
        url: `${API_URL}/${settings.joinClassEndpoint}/prize`,
        headers: {
          Authorization: `Bearer ${localStorage.getItem('apitoken')}`,
        },
        dataType: 'JSON',
...
<div style="margin-top: 4vh;">
      ...
      $('.payWarning').click(function (e) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              type: 'get',
              url: `${API_URL}/${settings.payNotificationEndpoint}`,
              headers: {
                Authorization: `Bearer ${localStorage.getItem('apitoken')}`,
              },
              data: {
    ...
    function editPayModal() {
      ...
      <select id="paidStatus" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
        ...
        str += `>${i18n.t(settings.paidStatusLabels.unpaid)}</option>
        <option value="1"`;
        if (tempPay[id]['paidStatus'] == 1) {
          str += 'selected';
        }
        str += `>${i18n.t(settings.paidStatusLabels.paid)}</option>
      </select>
      </div>
  </div>
  </form>
  <div style="margin-top: 4vh;">
    ...
    function editPay(i, id) {
      $('.submitEditPayBtn').click(function (e) {
        ...
        if (!status || status === '') {
          ...
          Swal.fire({
            title: i18n.t('confirm.title'),
            text: i18n.t(settings.confirmEditKey, {
              name: tempPay[i]['name'],
    ...
    function deletePay() {
      $('.deletePayBtn').click(function (e) {
        ...
        Swal.fire({
          title: i18n.t('confirm.title'),
          text: i18n.t(settings.confirmDeleteKey, {
