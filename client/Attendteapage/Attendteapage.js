...
function loadData() {
  $.ajax({
    type: "get",
    url: API_URL + "/teacherjoinclasses/date",
    headers: {
      Authorization: "Bearer " + localStorage.getItem("apitoken"),
    },
    dataType: "JSON",
      ...
      res.data.forEach((el, index) => {
        ...
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-center">
          ...
          `
                        ...
                        `;
        if (el["status"] == 1) {
          str += i18n.t('status.taught');
        }
        if (el["status"] == 0) {
          str += i18n.t('status.teaching');
        } else if (el["status"] == -1) {
          str += i18n.t('status.not_taught');
        }
        str +=
          `
                        </td>
...
function addDate() {
  $(".submitAddBtn").click(function (e) {
    ...
    if (!className || className === "") {
      ...
      $.ajax({
        type: "post",
        url: API_URL + "/teacherjoinclasses/class",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("apitoken"),
        },
        data: {
  ...
  $(".editBtn").click(function (e) {
    ...
    str +=
      ...
      `" required>
                    ...
                    <div>
                        <label for="status"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">` + i18n.t('label.status') + `</label>
      ...
      `>` + i18n.t('status.not_taught') + `</option>
                            <option value="0" ` +
      (temp[id]["status"] === 0 ? "selected" : "") +
      `>` + i18n.t('status.teaching') + `</option>
                            <option value="1" ` +
      (temp[id]["status"] === 1 ? "selected" : "") +
      `>` + i18n.t('status.taught') + `</option>
